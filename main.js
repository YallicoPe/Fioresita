...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "DragonGit"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './DragonGit';
...
const mainBotAuthFile = 'DragonSession';
try {
...
let prekey = []
let directorio = readdirSync("./DragonSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DragonSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./DragonGit/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./DragonGit/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./DragonGit/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./DragonGit/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./DragonSession/', './DragonGit/']
